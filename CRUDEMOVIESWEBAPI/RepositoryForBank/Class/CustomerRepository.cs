    ...
    public class CustomerRepository: ICustomerRepository
    {
        ...
        public async Task<int> Trnsaction(int cId, double ammount,string transactionType)
        {
            ...
            using(var connection=_dapperContext.CreateConnection())
            {
                ...
                { 
                    var addammount = await connection.QuerySingleAsync<double>(@"select accountBalance from tblCustomer where cId=@cId", new { cId = cId });
                    if(addammount < ammount)
                    {
                        var cbill = await connection.QuerySingleAsync<int>(@"select cibilScore from tblCustomer where cId=@cId", new { cId = cId });
                        if(cbill>700)
                        {
                            addammount -= ammount;
                            var rest = await connection.ExecuteAsync(query, new { cId = cId, accountBalance = addammount });
                            return -4;
                        }
                        return -3;
                    }
